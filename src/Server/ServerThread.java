...
public class ServerThread extends Thread {
	...
	private void kommunizieren() {
		AdminGUI.consoleText("Kommunikation mit Client ("
				+ client.getLocalAddress().getHostName()
				+ ") wird aufgenommen!");
		try {
			...
			br = new BufferedReader(new InputStreamReader(client
					.getInputStream()));
			pw = new PrintWriter(new OutputStreamWriter(client
	...
	private void sendeListe() throws IOException {
		...
		if (br.readLine().equals("EmpfangeScores")) {
			...
			oos
					.writeObject(ScoreServer.getList()[avNummer - 1]
							.getHighscores());
	...
	private void scoreEinfügen() throws IOException {
		...
		if (br.readLine().equals("SendeScore")) {
			...
			AdminGUI
					.consoleText("Highscore eingefügt: " + name + " | " + score);
