...
public class Verzweigung extends JPanel implements MouseListener,
		ActionListener { // wahrscheinlich müssen wir hier später MouseListener
							// nehmen, aber egal
	...
	public Verzweigung(String name, String beschreibung, ArrayList<Aktion> akt,
			ArrayList<Verzweigung> verz) {
		this.name = name;
		...
		// wir sollten Buttons für Verzweigungen und für Aktionen wahrscheinlich
		// noch unterschiedliche Farben geben
		...
		for (int i = 0; i < verzweigungen.size(); i++) {
			...
			verzweigungsButtons.get(i).setBorder(
					BorderFactory.createEtchedBorder());
			verzweigungsButtons.get(i).setOpaque(true);
			if (aktionen.size() == 0)
				verzweigungsButtons.get(i).setLocation(10,
						((aktionen.size() + i) * 30) + 40);
			else
				verzweigungsButtons.get(i).setLocation(10,
						((aktionen.size() + i) * 30) + 40);
	...
	// setVisible überschreiben, damit alle Untermenus dieser Verzweigung auch
	// ausgeblendet werden
	...
	// isVisible überschreiben, damit Spielbereich erfahren kann, ob noch
	// Untermenus dieses menus visible sind
	...
	public void mouseClicked(MouseEvent arg0) {
		...
		} else if (arg0.getSource().getClass().getName().split("\\.")[2]
				.equals("Aktion")) {
			for (int i = 0; i < aktionen.size(); i++) {
	...
	public void mouseEntered(MouseEvent arg0) {
		if (arg0.getSource() == this)
			return;
		if (arg0.getSource().getClass().getName().split("\\.")[2]
				.equals("Aktion")) {
			for (int i = 0; i < aktionen.size(); i++) {
				if (aktionen.get(i).equals(arg0.getSource())) {
			...
			for (int i = 0; i < verzweigungen.size(); i++) {
				if (verzweigungsButtons.get(i).equals(arg0.getSource())) {
					beschreibungFuellen("(blue)"
							+ verzweigungen.get(i).getBeschreibung());
					return;
				}
			}
		}
	...
	public void beschreibungFuellen(String str) {
		...
		for (int i = 0; i < beschr.length; i++) {
			...
			try {
				sd.insertString(sd.getLength(), besch[1], beschreibungsfeld
						.getStyle(besch[0]));
			} catch (Exception e) {
			}
		}
