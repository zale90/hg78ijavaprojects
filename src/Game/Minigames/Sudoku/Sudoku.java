...
public class Sudoku extends JFrame implements ActionListener, MouseListener
{
  ...
  // Anfang Attribute
   ...
   private String einl;
   private static String übergabe;
   private Spiel spiel;
  // Ende Attribute
   ...
   public Sudoku(String grad, Spiel spiel)
   {
      ...
      icon.setImage(icon.getImage().getScaledInstance(500,700,Image.SCALE_DEFAULT));   //passt das Bild auf das JLabel an
      ...
      for(int zeile = 0; zeile < 9; zeile++)
      {
        for(int spalte = 0; spalte < 9; spalte++)
        {
          ...
          felder[zeile][spalte].setBounds(50+spalte*28,80+zeile*28,25,25);
          felder[zeile][spalte].setHorizontalAlignment(0);
          felder[zeile][spalte].setCaretColor(new Color(16,78,139)); // ändert Farbe des Cursors
          felder[zeile][spalte].setBackground(Color.gray); // ändert Hintergrundfarbe des Textfeldes
          felder[zeile][spalte].setForeground(new Color(127,255,0));  // ändert die Schriftfarbe
          felder[zeile][spalte].setFont(new Font("Arial Black", 4,20));  // ändert Schriftstil; Schriftgröße
           ((AbstractDocument) felder[zeile][spalte].getDocument()).setDocumentFilter(new DocumentSizeFilter(1, "[1-9]"));
     ...
     for(int zeile = 0; zeile < 9; zeile++)
     {
       for(int spalte = 0; spalte < 9; spalte++)
       {
         if(grad.equals("Leicht"))
         {
           ...
           felder[zeile][spalte].setText(speicher.getStartLeicht(zeile, spalte));
         }
         else if(grad.equals("Mittel"))
         {
           speicher.startenMittel();
           felder[zeile][spalte].setText(speicher.getStartMittel(zeile, spalte));
         }
         if(!felder[zeile][spalte].getText().equals(""))
         {
           felder[zeile][spalte].setForeground(new Color(255,193,37));
           felder[zeile][spalte].setEditable(false);
         }
         if(felder[zeile][spalte].getText().equals(""))
         {
           felder[zeile][spalte].setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)); // Maus wird zu Hand
         }
